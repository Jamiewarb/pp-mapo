...
$title = "BOPIS Test "; 
$sel = "bopis"; // This decides which page in the sidebar is selected. Edit the sidebar.php include to check this value for each li
...
if ($stmt = $mysqli->prepare("SELECT `id` FROM `transactions` WHERE `type` = 'Sale';")) {
	$stmt->execute();
	$stmt->store_result();
...
if ($stmt = $mysqli->prepare("SELECT `id`, `product`, `type`, `trans_id`, `amt`, `datetime` FROM `transactions` WHERE `type` = 'Sale' ORDER BY `datetime` DESC LIMIT ?, ?;")) {
...
<?php include "$sitepath/resources/includes/header.php"; ?>
		<section>
			...
			<div class="content">
				...
				<form method="post" action="resources/processing/ecsalebopisprocess.php">
					<div class="clear"></div>
					...
					<input type="image" class="checkout" src="https://www.paypalobjects.com/en_GB/GB/i/btn/btn_xpressCheckout.gif" name="submit">
				</form>
				<div class="clear"></div>
