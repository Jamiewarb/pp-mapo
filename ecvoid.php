...
$title = "EC Voids - ";
$sel = "ecvoid"; // This decides which page in the sidebar is selected. Edit the sidebar.php include to check this value for each li
if (isset($_GET['page'])) {
	$page = $_GET['page'];
} else {
	$page = 1;
...
if ($stmt = $mysqli->prepare("SELECT `id` FROM `transactions` WHERE `type` = 'Authorization' OR `type` = 'Voided';")) {
	$stmt->execute();
	$stmt->store_result();
...
if ($stmt = $mysqli->prepare("SELECT `id`, `product`, `type`, `trans_id`, `amt`, `datetime` FROM `transactions` WHERE `type` = 'Authorization' OR `type` = 'Voided' ORDER BY `datetime` DESC LIMIT ?, ?;")) {
				...
				<table class="rounded blue full-width">
					...
					<?php if ($num_rows > 0) {
						foreach($transactions as $trans) {
							...
							if ($trans['type'] == "Voided") {
								$type_text = "<div style=\"color:red\">".$trans['type']."</div>";
								$void = "";
							} else {
								$void = "<a class=\"refund\" href=\"/mapo/resources/processing/ecvoidprocess.php?transid=".$trans['trans_id']."\">Void</a>";
								$type_text = $trans['type'];
							}
							echo "<tr><td>".$trans['id']." <td>".$trans['product']." <td>".$type_text." <td>".$trans['trans_id']." <td>£".$trans['amt']." <td>".$dt." <td>$void";
						} 
					} else {
						echo "<tr><td colspan=\"42\">There are no fields that meet the search criteria (Type = Voided or Authorized)";
					} ?>
					</tbody>
				</table>
			</div>
