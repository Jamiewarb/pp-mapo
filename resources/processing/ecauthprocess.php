...
$PayPalReturnURL = 'http://localhost/mapo/resources/processing/ecauthprocess.php'; //Point to process.php page
$PayPalCancelURL = 'http://localhost/mapo/ecauth.php?s=cancel'; //Cancel URL if user clicks cancel
...
if ($_POST) {
	...
	$GrandTotal = ($ItemTotalPrice + $TotalTaxAmount + $ShippingCost + $HandlingCost);
	...
	$padata = 	'&RETURNURL='.urlencode($PayPalReturnURL).
				...
				*/
				...
				$_SESSION['HandlingCost'] 		=  $HandlingCost; //Although you may change the value later, try to pass in a shipping amount that is reasonably accurate.
...
if (isset($_GET["token"]) && isset($_GET["PayerID"])) {
	...
	$HandlingCost 		= $_SESSION['HandlingCost']; //Although you may change the value later, try to pass in a shipping amount that is reasonably accurate.
	...
	if ("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) {
		...
		$trans_id = urldecode($httpParsedResponseAr["PAYMENTINFO_0_TRANSACTIONID"]);
		/*
		...
		*/
		...
		if ('Completed' == $httpParsedResponseAr["PAYMENTINFO_0_PAYMENTSTATUS"]) {
			...
			echo '<div style="color:red">Authorization Transaction Complete. Payment has not yet been captured.</div>';
		}
		echo "<a href=\"ecauthprocess.php?capture=$trans_id\">Click me to Capture Payment</a>";
		...
		if ("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) {
			...
			if ($stmt = $mysqli->prepare("SELECT `id` FROM `transactions` WHERE `trans_id` = ?;")) {
			...
			if ($saved == 0) {
				if ($stmt = $mysqli->prepare("INSERT INTO `transactions` (`payer_id`, `product`, `token`, `type`, `trans_id`, `amt`, `address`, `datetime`) VALUES (?, ?, ?, ?, ?, ?, ?, ?);")) {
				    if ($stmt->bind_param('ssssssss', $payer_id, $product, $token, $type, $trans_id, $amt, $address, $dt)) {
					    if ($stmt->execute()) {
					    	echo "<br><div style=\"color:green\">This transaction has been successfully saved to the database!</div>";
					    } else {
							echo "<br><div style=\"color:red\">Error: Transaction failed to save to the database!<br>".$mysqli->error."</div>";
						}
					} else {
						echo "<br><div style=\"color:red\">Error: Transaction failed to save to the database!<br>".$mysqli->error."</div>";
					}
			...
			$_SESSION['token']			=  urldecode($httpParsedResponseAr["TOKEN"]);
			$_SESSION['payer_id']		=  urldecode($httpParsedResponseAr["PAYERID"]);
			$_SESSION['trans_id']		=  $trans_id;
			$_SESSION['currency_code']	=  urldecode($httpParsedResponseAr["PAYMENTREQUEST_0_CURRENCYCODE"]);
			$_SESSION['amt']			=  urldecode($httpParsedResponseAr["AMT"]);
		} else  {
	...
	$trans_id = $_GET['capture'];
	if ($stmt = $mysqli->prepare("SELECT `token`, `payer_id`, `amt` FROM `transactions` WHERE `trans_id` = ?;")) {
		$stmt->bind_param("s", $trans_id);
		$stmt->execute();
		$stmt->bind_result($token, $payer_id, $amt);
		$stmt->fetch();
		$stmt->close();
	}
	$currency_code = "GBP";
	...
	$padata = 	'&AUTHORIZATIONID='.urlencode($trans_id).
				...
				'&COMPLETETYPE='.urlencode("Complete");

	$paypal = new MyPayPal();
	$httpParsedResponseAr = $paypal->PPHttpPost('DoCapture', $padata, $PayPalApiUsername, $PayPalApiPassword, $PayPalApiSignature, $PayPalMode);	

	if ("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) {
		
		if ($stmt = $mysqli->prepare("UPDATE `transactions` SET `type` = \"Captured\" WHERE `trans_id` = ?;")) {
			$stmt->bind_param("s", $trans_id);
			$stmt->execute();
			$stmt->close();
		} else {
		...
		echo '<h2>Success</h2>';
		echo 'Your Transaction ID : '.urldecode($httpParsedResponseAr["PAYMENTINFO_0_AUTHORIZATIONID"]);
		echo "<div style=\"color:green\">Successfully Captured Payment, and updated database</div>";
