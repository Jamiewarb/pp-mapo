...
$PayPalReturnURL = 'http://localhost/mapo/resources/processing/iccsaleprocess.php'; //Point to process.php page
$PayPalCancelURL = 'http://localhost/mapo/icc-implement.php?s=cancel'; //Cancel URL if user clicks cancel
...
if ($_POST) {
	...
	$padata = 	'&RETURNURL='.urlencode($PayPalReturnURL).
				...
				*/
				...
				//'&LOGOIMG=https://localhost/mapo/resources/images/pp_sig.png'. //site logo
				//'&CARTBORDERCOLOR=0079C1'. //border color of cart
				'&ALLOWNOTE=0'; //Allow the buyer to add a note to the merchant
	...
	if ("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) {
		...
		$paypalurl ='https://www'.$paypalmode.'.paypal.com/checkoutnow?token='.$httpParsedResponseAr["TOKEN"].'';
	...
	if ("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) {
		...
		*/
		...
		if ("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) {
			...
			if ($stmt = $mysqli->prepare("SELECT `id` FROM `icctrans` WHERE `trans_id` = ?;")) {
			...
			if ($saved == 0) {
				if ($stmt = $mysqli->prepare("INSERT INTO `icctrans` (`payer_id`, `product`, `token`, `type`, `trans_id`, `amt`, `address`, `datetime`) VALUES (?, ?, ?, ?, ?, ?, ?, ?);")) {
